...
/**
 * One chat bubble (assistant or user).
 * Now supports inline images via `message.imgSrc` + debug logs.
 */
    ...
    return (
      ...
      >
        <div className="flex gap-4">
          ...
          {/* Debug frame so you can see the image area even if it fails */}
          <div style={{ background: '#111', padding: 8, borderRadius: 12 }}>
            <img
              src={message.imgSrc}
              alt="assistant image"
              className="rounded-lg max-w-full h-auto"
              onLoad={() => {
                const src = message.imgSrc || '';
                console.log('IMG loaded', src.slice(0, 32), 'len=', src.length);
              }}
              onError={(e) => {
                const src = e.currentTarget?.src || '';
                console.error('IMG error, src prefix:', src.slice(0, 64));
              }}
            />
          </div>
        </div>
